...
/*
* 톱니1은 2번 인덱스, 톱니2&3는 2, 6번 인덱스, 톱니4는 6번 인덱스
* 1. 입력 받으면, deque로 해당 톱니 해당 방향으로 돌림
* 2. 2, 6번 인덱스 변경 여부 확인
* 2-1. 그렇다면 바뀐방향 톱니 반대방향으로 바꿈
* */
        ...
        for (int i = 0; i < n; i++) {
            ...
            while (!willBeRotate.isEmpty()) {
                ...
                for (int j = 0; j < 2; j++) {
                    ...
                    if (tmpIdx < 1 || tmpIdx > 4) continue;

                    if (dx[j] == 1) {
                        if (gears[tmpGear.idx].get(2) != gears[tmpIdx].get(6)) willBeRotate.offer(new GearSecond(tmpIdx, (-1)* tmpGear.dir));
                    } else {
                        if (gears[tmpGear.idx].get(6) != gears[tmpIdx].get(2)) willBeRotate.offer(new GearSecond(tmpIdx, (-1)* tmpGear.dir));
                    }
                }
            }
//            for (int k = 1; k < 5; k++) {
